...
将 [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) 服务器的工具桥接到 YueLiBot，使月璃能够调用外部 MCP 工具。
...
```bash
# 克隆到 YueLiBot 插件目录
cd /path/to/YueLiBot/plugins
git clone https://github.com/CharTyr/YueLiBot_MCPBridgePlugin.git MCPBridgePlugin
...
### 更新日志
- 见 `plugins/YueLiBot_MCPBridgePlugin/CHANGELOG.md`
...
**启用方式：**
在 WebUI「ReAct (软流程)」配置区启用，MCP 工具将自动注册到 YueLiBot 的记忆检索 ReAct 系统。
