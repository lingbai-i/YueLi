"""
MCP 桥接插件 v2.0.0
将 MCP (Model Context Protocol) 服务器的工具桥接到 YueLiBot
...
def convert_json_type_to_tool_param_type(json_type: str) -> ToolParamType:
    """将 JSON Schema 类型转换为 YueLiBot 的 ToolParamType"""
    type_mapping = {
...
def parse_mcp_parameters(input_schema: Dict[str, Any]) -> List[Tuple[str, ToolParamType, str, bool, Optional[List[str]]]]:
    """解析 MCP 工具的参数 schema，转换为 YueLiBot 的参数格式"""
    parameters = []
...
• 兼容旧版：YueLiBot servers 列表数组（将自动迁移为 mcpServers）
...
class MCPBridgePlugin(BasePlugin):
    """MCP 桥接插件 v2.0.0 - 将 MCP 服务器的工具桥接到 YueLiBot"""
    ...
    config_schema: dict = {
        ...
        "settings": {
            ...
            "trace_log_enabled": ConfigField(
                ...
                label="📝 追踪日志文件",
                hint="启用后记录写入 plugins/YueLiBot_MCPBridgePlugin/logs/trace.jsonl",
                order=32,
            ),
            # v1.4.0 缓存配置
      ...
      {"tool_name": "mcp_server_get_detail", "args_template": {"id": "${step.search_result}"}}
        ...
        "react": {
            "react_enabled": ConfigField(
                ...
                label="🔄 启用 ReAct 集成",
                hint="启用后，YueLiBot 的 ReAct Agent 可在记忆检索时调用 MCP 工具",
                order=1,
            ),
...
""",
    ...
    def _register_tool_chains(self) -> None:
        """v1.8.1: 将工具链注册到 YueLiBot 组件系统，使 LLM 可调用"""
        from src.plugin_system.core.component_registry import component_registry
    ...
    def _register_tools_to_react(self) -> int:
        ...
        这样 YueLiBot 的 ReAct Agent 在检索记忆时可以调用 MCP 工具，
        实现 LLM 自主决策的多轮工具调用。
