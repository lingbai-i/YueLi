        ...
        while self.should_continue:
            ...
            try:
                ...
                if hasattr(self.observation_info, "new_messages_count"):
                    initial_new_message_count = self.observation_info.new_messages_count + 1  # 算上月璃自己发的那一条
                else:
                    logger.warning(
    ...
    async def _handle_action(
        ...
        if action == "send_new_message":
            ...
            if is_suitable:
                ...
                # --- VTB Stream Voice ---
                try:
                    asyncio.create_task(AudioManager().speak(self.generated_reply))
                except Exception as tts_e:
                    logger.error(f"[私聊][{self.private_name}]TTS启动失败: {tts_e}")
                # ------------------------
