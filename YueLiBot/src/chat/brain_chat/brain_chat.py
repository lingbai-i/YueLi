...
class BrainChatting:
    ...
    async def _loopbody(self):  # sourcery skip: hoist-if-from-if
        ...
        if len(recent_messages_list) >= 1:
            ...
            if self.audio_manager:
                logger.info(f"{self.log_prefix} 检测到新消息，停止音频播放")
    ...
    async def _send_and_store_reply(
        ...
        logger.info(f"{self.log_prefix} [AudioDebug] 进入 _send_and_store_reply 方法")
        ...
        logger.info(f"{self.log_prefix} [PlatformDebug] 检测到的平台: '{platform}'")

        # 预先构建回复文本，因为后面 TTS 和 消息发送 都需要它
        reply_text = ""
        for reply_content in response_set.reply_data:
            if reply_content.content_type == ReplyContentType.TEXT:
                 reply_text += str(reply_content.content)

        # -------------------------------------------------------------------------
        # 核心音频子系统集成 (Core Audio Subsystem Integration)
        # -------------------------------------------------------------------------
        try:
            # 仅当配置了 voice.vtb 且输出设备有效时才播放
            # AudioManager 内部会处理文本分句和流式合成
            
            logger.info(f"{self.log_prefix} [AudioDebug] 准备触发语音，文本长度: {len(reply_text) if reply_text else 0}, 设备: {self.audio_manager.output_device_name}")
            
            # 简单的检查：如果 text 存在且 AudioManager 已初始化
            if reply_text:
                 if self.audio_manager.output_device:
                     # 使用 asyncio.create_task 异步播放，不阻塞回复流程
                     asyncio.create_task(self.audio_manager.speak(reply_text))
                     logger.info(f"{self.log_prefix} 已触发核心语音流式合成: {reply_text[:10]}...")
                 else:
                     logger.warning(f"{self.log_prefix} [AudioDebug] 音频输出设备未初始化，无法播放")
            else:
                 logger.warning(f"{self.log_prefix} [AudioDebug] 回复文本为空，跳过语音")
                 
        except Exception as e:
            logger.error(f"{self.log_prefix} 核心语音触发失败: {e}", exc_info=True)
        # -------------------------------------------------------------------------
        ...
        with Timer("回复发送", cycle_timers):
            ...
            # 如果是 Bilibili 平台，强制只使用语音
            # 兼容可能的平台标识：bilibili, Bilibili直播间, live_bilibili 等
            if platform and "bilibili" in str(platform).lower():
                only_voice = True
                logger.info(f"{self.log_prefix} 检测到 Bilibili 平台 (ID: {platform})，自动启用仅语音模式")
            
            # 兼容配置项 voice.only_voice
            if not only_voice:
                try:
                    # 检查配置 voice.only_voice
                    voice_config = getattr(global_config, "voice", {})
                    if hasattr(voice_config, "only_voice"):
                        if voice_config.only_voice:
                            only_voice = True
                    elif isinstance(voice_config, dict):
                         if voice_config.get("only_voice", False):
                            only_voice = True
                except Exception:
                    pass
            
            if only_voice:
                logger.info(f"{self.log_prefix} 启用仅语音模式，跳过文本消息发送")
                # 即使不发送，我们也认为发送成功，以便记录日志和历史
                pass
            else:
                # _send_response 仍然返回 text，但我们已经提前计算了 reply_text
                # 这里主要利用它的发送副作用
        ...
        # 获取 platform，如果不存在则从 chat_stream 获取，如果还是 None 则使用默认值
        # platform 变量已经在上面获取并使用了，这里不需要再次获取
        # platform = action_message.chat_info.platform
        # if platform is None:
        #    platform = getattr(self.chat_stream, "platform", "unknown")
    ...
    async def _main_chat_loop(self):
        ...
        try:
            ...
            # 设置了关闭标志位后被取消是正常流程
            logger.info(f"{self.log_prefix} 月璃已关闭聊天")
        except Exception:
            logger.error(f"{self.log_prefix} 月璃聊天意外错误，将于3s后尝试重新启动")
