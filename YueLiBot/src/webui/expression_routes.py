...
async def get_chat_list(YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)):
    """
    获取所有聊天列表（用于下拉选择）
...
async def get_expression_detail(
    expression_id: int, YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)
):
    """
    获取表达方式详细信息
