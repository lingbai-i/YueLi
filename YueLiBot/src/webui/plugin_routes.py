...
def get_token_from_cookie_or_header(
    YueLiBot_session: Optional[str] = None,
    authorization: Optional[str] = None,
) -> Optional[str]:
    """从 Cookie 或 Header 获取 token"""
    # 优先从 Cookie 获取
    if YueLiBot_session:
        return YueLiBot_session
    # 其次从 Header 获取
...
class VersionResponse(BaseModel):
    ...
    version: str = Field(..., description="月璃版本号")
...
async def get_YueLi_version() -> VersionResponse:
    """
    获取月璃版本信息
...
async def get_available_mirrors(
    YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)
) -> AvailableMirrorsResponse:
    """
    获取所有可用的镜像源配置
    """
    # Token 验证
    token = get_token_from_cookie_or_header(YueLiBot_session, authorization)
...
async def add_mirror(
    request: AddMirrorRequest, YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)
) -> MirrorConfigResponse:
    """
    添加新的镜像源
    """
...
async def update_mirror(
    ...
    YueLiBot_session: Optional[str] = Cookie(None),
...
async def delete_mirror(
    mirror_id: str, YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)
) -> Dict[str, Any]:
    """
    删除镜像源
    """
...
async def get_local_plugin_readme(
    plugin_id: str, YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)
) -> Dict[str, Any]:
    """
