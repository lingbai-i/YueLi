...
async def restart_YueLiBot(_auth: bool = Depends(require_auth)):
    """
    重启月璃主程序
    ...
    try:
        ...
        # 立即返回成功响应
        return RestartResponse(success=True, message="月璃正在重启中...")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"重启失败: {str(e)}") from e


@router.get("/status", response_model=StatusResponse)
async def get_YueLiBot_status(_auth: bool = Depends(require_auth)):
    """
    获取月璃运行状态
