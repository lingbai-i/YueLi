...
async def health_check():
    """健康检查"""
    return {"status": "healthy", "service": "YueLiBot WebUI"}
...
async def check_auth_status(
    ...
    try:
        ...
        # 记录请求信息用于调试
        logger.debug(f"检查认证状态 - Cookie: {YueLiBot_session[:20] if YueLiBot_session else 'None'}..., Authorization: {'Present' if authorization else 'None'}")

        # 优先从 Cookie 获取
        if YueLiBot_session:
            token = YueLiBot_session
...
async def update_token(
    ...
    try:
        ...
        current_token = None
        if YueLiBot_session:
            current_token = YueLiBot_session
        elif authorization and authorization.startswith("Bearer "):
