...
def verify_auth_token(
    ...
    return verify_auth_token_from_cookie_or_header(YueLiBot_session, authorization)
...
async def get_emoji_list(
    ...
    """
    try:
        verify_auth_token(YueLiBot_session, authorization)
...
async def get_emoji_detail(
    emoji_id: int, YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)
):
    """
    获取表情包详细信息
...
async def get_emoji_stats(YueLiBot_session: Optional[str] = Cookie(None), authorization: Optional[str] = Header(None)):
    """
    获取表情包统计数据
...
async def get_emoji_thumbnail(
    ...
    Args:
        ...
        YueLiBot_session: Cookie 中的 token
        authorization: Authorization header
    ...
    try:
        ...
        # 1. 优先使用 Cookie
        if YueLiBot_session and token_manager.verify_token(YueLiBot_session):
            is_valid = True
