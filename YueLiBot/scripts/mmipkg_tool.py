    ...
    def pack_from_db(
        ...
        try:
            ...
            if pack_name is None:
                pack_name = f"YueLiBot_Emojis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def print_header():
    ...
    console.print("[bold cyan]" + " " * 20 + "YueLiBot 表情包打包工具" + "[/bold cyan]")
...
def interactive_export():
    ...
    default_filename = f"YueLiBot_emojis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mmipkg"
    ...
    # 获取包名称
    default_pack_name = f"YueLiBot表情包_{datetime.now().strftime('%Y%m%d')}"
