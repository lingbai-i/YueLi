...
SERVICE_NAME_NBADAPTER="YueLiBot-napcat-adapter"
...
check_eula() {
    # 首先计算当前EULA的MD5值
    current_md5=$(md5sum "${INSTALL_DIR}/YueLiBot/EULA.md" | awk '{print $1}')

    # 首先计算当前隐私条款文件的哈希值
    current_md5_privacy=$(md5sum "${INSTALL_DIR}/YueLiBot/PRIVACY.md" | awk '{print $1}')
    ...
    if [[ -f ${INSTALL_DIR}/YueLiBot/eula.confirmed ]]; then
        # 如果存在则检查其中包含的md5与current_md5是否一致
        confirmed_md5=$(cat ${INSTALL_DIR}/YueLiBot/eula.confirmed)
    else
        confirmed_md5=""
    fi

    # 检查privacy.confirmed文件是否存在
    if [[ -f ${INSTALL_DIR}/YueLiBot/privacy.confirmed ]]; then
        # 如果存在则检查其中包含的md5与current_md5是否一致
        confirmed_md5_privacy=$(cat ${INSTALL_DIR}/YueLiBot/privacy.confirmed)
    else
    ...
    if [[ $current_md5 != $confirmed_md5 || $current_md5_privacy != $confirmed_md5_privacy ]]; then
        whiptail --title "📜 使用协议更新" --yesno "检测到MaiCore EULA或隐私条款已更新。\nhttps://github.com/MaiM-with-u/YueLiBot/blob/refactor/EULA.md\nhttps://github.com/MaiM-with-u/YueLiBot/blob/refactor/PRIVACY.md\n\n您是否同意上述协议？ \n\n " 12 70
        if [[ $? -eq 0 ]]; then
            echo -n $current_md5 > ${INSTALL_DIR}/YueLiBot/eula.confirmed
            echo -n $current_md5_privacy > ${INSTALL_DIR}/YueLiBot/privacy.confirmed
        else
            exit 1
        fi
    fi
...
run_installation() {
    ...
    git clone -b "$BRANCH" "$GITHUB_REPO/MaiM-with-u/YueLiBot" YueLiBot || {
    ...
    echo -e "${GREEN}克隆 nonebot-plugin-YueLiBot-adapters 仓库...${RESET}"
    git clone $GITHUB_REPO/MaiM-with-u/YueLiBot-Napcat-Adapter.git || {
        echo -e "${RED}克隆 YueLiBot-Napcat-Adapter.git 仓库失败！${RESET}"
        exit 1
    }


    echo -e "${GREEN}安装Python依赖...${RESET}"
    pip install -r YueLiBot/requirements.txt
    cd YueLiBot
    pip install uv
    ...
    echo -e "${GREEN}部署YueLiBot Napcat Adapter...${RESET}"
    cd YueLiBot-Napcat-Adapter
    uv pip install -i https://mirrors.aliyun.com/pypi/simple -r requirements.txt
    cd ..
    ...
    # 首先计算当前EULA的MD5值
    current_md5=$(md5sum "YueLiBot/EULA.md" | awk '{print $1}')

    # 首先计算当前隐私条款文件的哈希值
    current_md5_privacy=$(md5sum "YueLiBot/PRIVACY.md" | awk '{print $1}')

    echo -n $current_md5 > YueLiBot/eula.confirmed
    echo -n $current_md5_privacy > YueLiBot/privacy.confirmed
...
[Service]
Type=simple
WorkingDirectory=${INSTALL_DIR}/YueLiBot
...
[Unit]
Description=YueLiBot Napcat Adapter
After=network.target mongod.service ${SERVICE_NAME}.service

[Service]
Type=simple
WorkingDirectory=${INSTALL_DIR}/YueLiBot-Napcat-Adapter
