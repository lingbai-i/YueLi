...
is_worker = os.environ.get("YueLiBot_WORKER_PROCESS") == "1"
...
# 检查是否是 Worker 进程
# 如果没有设置 YueLiBot_WORKER_PROCESS 环境变量，说明是直接运行的脚本，
# 此时应该作为 Runner 运行。
if os.environ.get("YueLiBot_WORKER_PROCESS") != "1":
...
def print_opensource_notice():
    ...
    notice_lines = [
        ...
        f"{Fore.GREEN}  ★ YueLiBot - 开源 AI 聊天机器人 ★{Style.RESET_ALL}",
        f"{Fore.CYAN}{'─' * 70}{Style.RESET_ALL}",
        f"{Fore.YELLOW}  本项目是完全免费的开源软件，基于 GPL-3.0 协议发布{Style.RESET_ALL}",
        f"{Fore.WHITE}  如果有人向你「出售本软件」，你被骗了！{Style.RESET_ALL}",
        "",
        f"{Fore.WHITE}  官方仓库: {Fore.BLUE}https://github.com/MaiM-with-u/YueLiBot {Style.RESET_ALL}",
        f"{Fore.WHITE}  官方文档: {Fore.BLUE}https://docs.YueLi-YueLi.org {Style.RESET_ALL}",
...
async def graceful_shutdown():  # sourcery skip: use-named-expression
    try:
        ...
        logger.info("月璃优雅关闭完成")

    except Exception as e:
        logger.error(f"月璃关闭失败: {e}", exc_info=True)
